...
- **데이터 계약 선설계** 2인 팀에서 각자 만들고 나중에 합치는 방식을 피하려고, 코드 이전에 **모델의 출력 스키마와 서버의 입력 스키마를 문서로 확정** — 모델 정확도를 손보는 동안에도 서버 작업이 멈추지 않았습니다

- **주소 고정으로 장애 원인 제거** 재시작마다 바뀌던 ngrok 주소를 **Cloudflare Tunnel 고정 도메인**으로 전환. 배포마다 손대야 하던 갱신 지점이 **3곳 → 0곳**(프론트 `.env` · OAuth Redirect URI · 백엔드 CORS). 주소가 상수가 되면서 이 경로의 장애가 사라졌습니다

- **장애 1차 대응 절차화** 배포 로그 → 기본 실행 URL → 공개 URL → 프론트 요청 주소 → 서버 도달 → CORS·host **6단계 체크리스트**를 만들어, 증상 4종(`ERR_FAILED` · `ERR_NAME_NOT_RESOLVED` · 404 · CORS)이 **같은 원인**임을 특정했습니다. 이후 같은 증상은 매번 이 순서로 잡혔고 **트러블슈팅 문서 6건**이 쌓였습니다
