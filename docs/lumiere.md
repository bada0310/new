...
| 선택지 | 얻는 것 | 치르는 것 | 판정 |
|---|---|---|---|
| 로컬에서만 개발 | 비용 0 | 외부 API 콜백을 받을 수 없고 프론트와의 사전 통합 테스트도 불가 — **매번 배포한 뒤에야 확인** | 기각 |
| 클라우드에 올린다 | 주소가 고정되고 상시 가동 | **2인 프로젝트에서 쓸 수 없는 비용** | 기각 |
| **터널링으로 로컬을 노출** | 비용 없이 콜백과 사전 통합 테스트가 성립 | 로컬 머신이 꺼지면 서비스도 멈춤 — 데모·검증 단계에서만 성립 | **채택** |
...
| 선택지 | 얻는 것 | 치르는 것 | 판정 |
|---|---|---|---|
| ngrok 그대로 | 붙는 건 가장 빨랐음 | 서버를 재시작할 때마다 공개 주소가 바뀜 → **갱신 지점 3곳** | 교체 |
| 체크리스트를 더 꼼꼼히 | 도구를 바꾸지 않아도 됨 | 세 곳을 사람이 매번 틀리지 않아야 함 — 원인이 아니라 **조심에 기대는 방식** | 기각 |
| **Cloudflare Tunnel 고정 도메인** | 주소가 상수가 되어 **갱신 지점 3곳 → 0곳** | 로컬 머신이 꺼지면 멈추는 구조는 그대로 | **채택** |
...
CORS 설정 문제로만 봤습니다. **틀렸습니다** — CORS는 네 증상 중 하나였을 뿐이고, 넷 다 원인이 같았습니다.
