...
결과: 요구 기능이 **3개에서 6개로 늘었을 때** 기존 스토리에 추가만으로 대응했고 일정 지연이 없었습니다. 이후 프로젝트도 같은 방식으로 시작합니다.

### 핵심 성과 · Key Achievements

- **무중단 배포** 호스트 nginx에 포트가 하드코딩돼 제자리 교체되던 구조를 **blue-green**으로 재설계 — 배포 중 다운타임 **수 초 → 0**. health 폴링을 통과해야만 트래픽이 넘어가고, 실패하면 아무 일도 일어나지 않음

- **3D 에셋 파이프라인** 원본 GLB를 **38MB → 378KB(약 1/100)**로 경량화. POC 실측으로 소품당 **20K~50K 삼각형** 예산을 잡고 텍스처 프로필 3단계(21.3 / 42.6 / 47.9 MiB)를 비교해 MVP 기본값을 고정

- **선행 검증** 난도가 높은 GLB 변환·렌더링을 전체 개발 전에 POC로 떼어내 검증 — 핵심 기능 검증 기간 **7일 → 3일**
